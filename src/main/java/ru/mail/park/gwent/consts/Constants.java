package ru.mail.park.gwent.consts;

public class Constants {
    public static final String SESSION_USER_PROFILE_KEY = "UserProfile";
    public static final String SESSION_WEB_SOCKET_USER_KEY = "WebSocketUser";
    public static final String SIGN_UP_URL = "/api/join";
    public static final String AUTH_URL = "/api/auth";
    public static final String USERS_URL = "/api/users";

    public static final String ORIGIN = "https://tech-gwent.xyz";
    public static final String LOCALHOST = "http://localhost:8000";
    public static final String GWENT_FRONT = "https://gwent-front.herokuapp.com";
    public static final String TECH_GWENT = "https://techgwent.herokuapp.com";
}
